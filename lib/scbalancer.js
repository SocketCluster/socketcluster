...
var SCBalancer = function (options) {
  ...
  this.socketCookieName = options.socketCookieName;
  
  this._sidRegex = new RegExp('([^A-Za-z0-9]|^)' + this.socketCookieName + '=([^;]*)');
...
SCBalancer.prototype._parseSocketDest = function (req) {
  ...
  var matches = cookie.match(this._sidRegex);

  if (!matches) {
    return null;
  }

  var routeString = matches[2];
  
  var destSocketIndex = this._hash(routeString, this.workers.length);
  var destSocketName = this.workers[destSocketIndex];
