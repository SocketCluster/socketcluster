...
var SCServer = function (options) {
  ...
  this.deliveryTimeout = opts.deliveryTimeout;
...
SCServer.prototype._handleSocketConnection = function (wsSocket) {
  ...
  this.auth.parseToken(wsSocket.upgradeReq, function (err, authToken) {
      ...
      async.applyEachSeries(this._middleware[this.MIDDLEWARE_PUBLISH_IN], socket, data.channel, data.data,
        function (err) {
          if (err) {
            ...
            if (data.pid) {
              options.pid = data.pid;
            }
            self.global.publishRaw(data.channel, data.data, options, function (err) {
              cb(err);
            });
          }
        }
    ...
    async.applyEachSeries(this._middleware[this.MIDDLEWARE_PUBLISH_OUT], socket, data.channel, data.data,
      function (err) {
        if (err) {
          ...
          self._ioClusterClient.clearPubAck(socket.id, data.mid);
          cb(err);
        } else {
          cb();
        }
