...
var SCSocket = function (id, server, socket, authData) {
  ...
  var isPrivateTransmittedEvent = function (event) {
            ...
            if (err) {
              ...
              if (eventName == '#disconnect') {
                ...
                if (self._autoAckEvents[eventName]) {
                  response.end();
                  EventEmitter.prototype.emit.call(self, eventName, obj.data);
                } else {
                  EventEmitter.prototype.emit.call(self, eventName, obj.data, response.callback.bind(response));
                }
              }
            }
          });
