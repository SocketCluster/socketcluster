  ...
  this.socket.on('message', function (message, flags) {
    ...
    if (obj == null) {
      ...
      if (self._localEvents[obj.event] == null) {
        ...
        self.server.verifyEvent(self, obj.event, obj.data, function (err) {
          if (err) {
            response.error(err);
          } else {
      ...
      // If there is no pong property, then the last remaining case
      // is to treat this data as raw. Server pong is ignored by client.
      } else if (!obj.pong) {
