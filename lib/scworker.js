...
var SCWorker = function (options) {
  ...
  this._errorDomain.on('error', function () {
    ...
    // TODO: Some errors will be warnings and shouldn't crash the process.
    // Maybe they should have an 'isWarning' property.
...
SCWorker.prototype._init = function (options) {
  ...
  this._ioClusterClient.on('warning', function () {
    self.warningHandler.apply(self, arguments);
  });
  this.exchange = this.global = this._ioClusterClient.exchange();
  ...
  this.scServer = socketClusterServer.attach(this.httpServer, {
    brokerEngine: this._ioClusterClient,
...
SCWorker.prototype._startServer = function () {
  ...
  if (startMiddleware.length) {
    ...
    async.applyEachSeries(startMiddleware, options, function (err) {
      if (callbackInvoked) {
        // TODO: This needs to be a custom Error object with a name property
        self.emit('warning', new Error('Callback for ' + self.MIDDLEWARE_START + ' middleware was already invoked'));
      } else {
