...
SCWorker.prototype._init = function (options) {
  ...
  var oldRequestListeners = this.httpServer.listeners('request');
  this.httpServer.removeAllListeners('request');
  this.httpServer.on('request', this._httpRequestHandler.bind(this));
  for (var i in oldRequestListeners) {
    this.httpServer.on('request', oldRequestListeners[i]);
  }
...
SCWorker.prototype._start = function () {
  ...
  var serverOn = this.httpServer.on;
  this.httpServer.on = function (event) {
    if (event == 'request') {
      arguments[0] = 'req';
    } else if (event == 'rawRequest') {
      arguments[0] = 'request';
    }
    serverOn.apply(self.httpServer, arguments);
  };
...
SCWorker.prototype._httpRequestHandler = function (req, res) {
  ...
  if (req.url == this._paths.statusURL) {
    ...
    if (!this._socketPathRegex.test(req.url)) {
      this.httpServer.emit('req', req, res);
    }
  }
};
