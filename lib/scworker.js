...
SCWorker.prototype.init = function (options) {
  var self = this;
...
var handleWorkerClusterMessage = function (wcMessage) {
  if (wcMessage.type == 'init') {
    if (scWorkerIsInitialized) {
      throw new InvalidActionError('Attempted to initialize a worker which has already been initialized');
    }
    if (wcMessage.data.processTermTimeout) {
      processTermTimeout = wcMessage.data.processTermTimeout;
    }

    if (wcMessage.data && wcMessage.data.protocolOptions && wcMessage.data.protocolOptions.pfx) {
      wcMessage.data.protocolOptions.pfx = new Buffer(wcMessage.data.protocolOptions.pfx, 'base64');
    }

    if (typeof wcMessage.data.authKey === 'object' && wcMessage.data.authKey !== null && wcMessage.data.authKey.type === 'Buffer') {
      wcMessage.data.authKey = new Buffer(wcMessage.data.authKey.data, 'base64');
    }

    scWorker.init(wcMessage.data);

    if (wcMessage.data.propagateErrors) {
      scWorker.on('error', handleError.bind(null, scWorker.options.crashWorkerOnError));
      if (wcMessage.data.propagateWarnings) {
        scWorker.on('warning', handleWarning);
      }
      scWorker.on('exit', handleExit);
    }

    scWorker.on('ready', function () {
      scWorker.start();
      handleReady();
    });
  } else if (wcMessage.type == 'emit') {
    if (wcMessage.data) {
