...
SCWorker.prototype._init = function (options) {
  ...
  this.scServer = socketClusterServer.attach(this.httpServer, {
    ...
    defaultAuthTokenExpiry: this.options.defaultAuthTokenExpiry,
...
SCWorker.prototype._startServer = function () {
  ...
  if (startMiddleware.length) {
    ...
    async.applyEachSeries(startMiddleware, function (err) {
      if (callbackInvoked) {
        self.emit('notice', new Error('Callback for ' + self.MIDDLEWARE_START + ' middleware was already invoked'));
      } else {
